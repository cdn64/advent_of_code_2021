...
fn main() {
  ...
  let mut last_board_index: Option<usize> = None;
  ...
  for number in numbers {
    ...
    for board in 0..boards.len() {
      for win_condition in &win_conditions {
        ...
        if won && !winning_board_indices.contains(&board) {
          winning_board_indices.insert(board);
          if winning_board_indices.len() == boards.len() {
            last_board_index = Some(board);
    ...
    if last_board_index != None {
      break;
    }
  }

  // Sum up all unmarked numbers on last board
  let mut unmarked_sum = 0;
  let last_board = &boards[last_board_index.unwrap()];
  for y in 0..5 {
    for x in 0..5 {
      match last_board.0[y].0[x] {
  ...
  println!("Sum of unmarked numbers on last board: {}", unmarked_sum);
